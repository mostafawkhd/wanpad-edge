...
def client_program():
    with open('/etc/ssh/ssh_host_ecdsa_key.pub', 'r') as f:
        ...
        if request_to_url.status_code == 400:
            ...
            # restart filebeat
            time.sleep(2)
            os.system('service wanpad-filebeat restart') 
...
def client_request_to_gateway(scheme, gateway_address, token, dsf):
    ...
    if result.status_code == 200:
        ...
        controller = requests.post(f"{os.environ['CONTROLLER_PLUG_PLAY_URL']}/wanpad/api/v1/gateway/confirm_wireguard",
